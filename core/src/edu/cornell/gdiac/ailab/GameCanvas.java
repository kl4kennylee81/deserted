...
public class GameCanvas {
	...
	public void setShearBoard(float x,float y,float shearX,float shearY){
		...
		float translateBack = -this.getWidth()*0.006f;
	...
	public GlyphLayout drawTutorialText(String msg, Color color, int alignment) {
		...
		float x = GridBoard.BOARD_OFFSET_X * getWidth() + Constants.TILE_SHEAR*GridBoard.BOARD_HEIGHT*getHeight();
		float y = ((float)2*getHeight())/3f;
//		GlyphLayout g = displayFont.draw(spriteBatch, msg, x,y, width, alignment, true);
	...
	public void drawHighlightCharacter(Texture texture, float x, float y, Color color, float scale, boolean stretch){
