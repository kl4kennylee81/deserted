...
public class TutorialGameplayController extends GameplayController {
	...
	public TutorialGameplayController(MouseOverController moc, CompletionMenuController cmc, PauseMenuController pmc,
			FileHandle file, int fileNum) {
	...
	public boolean isDone() {
		...
		return ((tutorialSteps.finishGame && tutorialSteps.isDone() && inGameState != InGameState.PAUSED)
				|| super.isDone()) || endNow;
	}

	public void drawPlay(GameCanvas canvas) {

		// temporary hacky code to show that you have won without destroying the
		// canvas
		...
		if (tutorialSteps.step == null || !tutorialSteps.step.text.equals(""))
			screen.setJustScreen();
		...
		if (highlight_action > 0) {// must change
			...
			if (selectionMenuController.menuState != MenuState.PEEKING) {
				if (selectedChar != null) {
					...
					float highlightX = selectedChar.actionBar.getBarCastPoint(canvas)
							+ (highlight_action) * selectedChar.actionBar.getSlotWidth(canvas);
					float highlightY = selectedChar.actionBar.getY(canvas, count)
							- selectedChar.actionBar.getBarHeight(canvas);// characters.indexOf(selectedChar));
					if (selectionMenuController.menu != null && selectionMenuController.menu.actions != null
							&& selectionMenuController.menu.selectedAction != selectionMenuController.menu.actions.length) {
						canvas.drawDownTextArrow(highlightX + 20, highlightY + 8, Color.WHITE.cpy(),
								"Action executes here");
					}
				}
			}
			// getY: iterate over characters, and when character matches
			// selected character thats the number to pass to getY
		}
		...
		if (highlights != null && tutorialSteps.showHighlights) {
			for (CurrentHighlight highlight : highlights) {
				if (highlight.isChar) {
					...
					canvas.drawCharArrow(screen.screen, (float) highlight.xPos * canvas.getWidth(),
							(float) highlight.width * canvas.getWidth(), (float) highlight.yPos * canvas.getHeight(),
							(float) highlight.height * canvas.getHeight(), Color.WHITE.cpy(), board);
					continue;
				}
				if (highlight.arrow.equals("up")) {
					canvas.drawUpArrow(
							(float) (highlight.xPos * canvas.getWidth() + (highlight.width * canvas.getWidth()) / 2f),
							(float) highlight.yPos * canvas.getHeight(), Color.WHITE.cpy());

				} else if (highlight.arrow.equals("down")) {
					canvas.drawDownArrow(
							(float) (highlight.xPos * canvas.getWidth() + highlight.width * canvas.getWidth()),
							(float) (highlight.yPos * canvas.getHeight()
									+ (highlight.height * canvas.getHeight()) / 2f),
							Color.WHITE.cpy());
				} else {
					canvas.drawLeftArrow(
							(float) (highlight.xPos * canvas.getWidth() + highlight.width * canvas.getWidth()),
							(float) (highlight.yPos * canvas.getHeight()
									+ (highlight.height * canvas.getHeight()) / 2f),
							Color.WHITE.cpy());
		...
		if (tutorialSteps.currStep() != null) {
			...
			if (selectionMenuController.menu != null) {
				...
				for (Option o : selectionMenuController.menu.getOptions()) {
					if (highlightTokens != null && highlightTokens.contains(o.optionKey)
							&& tutorialSteps.showHighlights) {
		...
		if ((InputController.pressedEnter() || InputController.pressedLeftMouse())
			...
			if (tutorialSteps.textDone == tutorialSteps.step.text.length() || tutorialSteps.currStep().ignoreTextDone) {
				...
				if (tutorialSteps.step.text.charAt(tutorialSteps.textDone) == '\n') {
					...
					// System.out.println("prevTextDone:"+tutorialSteps.prevTextDone
					// + " textDone:" + tutorialSteps.textDone + " pt:" + pt + "
					// t:" + t);
					if (pt != -1)
						tutorialSteps.prevTextDone = pt == t ? tutorialSteps.prevTextDone : pt;
	...
	public void drawAfter(GameCanvas canvas) {
		canvas.drawTutorialText(
				"“Congratulations on completing this Tutorial segment. "
						+ "\n\nPress \'R\' to return to the main menu where you can "
						+ "select another level \n\nor Press Enter to move on to the next Tutorial segment",
